...
const AddressSchema = Yup.object().shape({
  ...
  phoneNumber: Yup.string()
    ...
    .min(10, "Phone Number is 10 digits")
    .max(10, "Phone Number is 10 digits"),
      ...
      <div className="container">
          ...
          <ul className="cart-items">
            {items.map((product) => {
              ...
              return (
                ...
                <li className="cart-item" key={product.name}>
                  <div className="product-info">Subtotal
                  <p className="amount" >{product.quantity * product.price}</p>
                    {/* <p className="product-price">{product.price}</p> */}
                  </div>
                  <li>
              ...
              <h2>Total</h2>
                <div className="product-info">Total
                  <p className="amount">{product.quantity * product.price + 10*(product.price*product.quantity)/100 }</p>
                </div>
              </li>
                </li>
              );
        ...
        {/* {items.map((product) => {
              return (
                <PayPalButton 
        options={{
            clientId: "AYmT5MggAkXXsi4xWgyejJ4XIY1WgmqqKzZQIQWUXU0pYoRZCwzyfMPHuGqZ7GvguHw1unSlI1VeSGK_",
            currency: "USD",
        }}
        amount={product.quantity * product.price + 10*(product.price*product.quantity)/100 }
        onSuccess={(details, data) => {
            alert("Get Pay: " + details.payer.name.given_name);
